...
public:
  ...
  public:
    ...
    std::string
    name(const KeyFormatter &keyFormatter = MultiRobotKeyFormatter) const;
  ...
  public:
    ...
    FactorGraphType marginal2() {
      FactorGraphType p_C;

      for (const auto &parent_edge : parentEdges()) {
        // get separator marginals from parent edges
        p_C.add(parent_edge->marginal());
        // get conditional from parent edges
        p_C.add(boost::shared_ptr<FactorType>(parent_edge->conditional()));
      }
  ...
  void
  saveGraph(const std::string &file_name,
            const CliqueSet &top_cliques = CliqueSet(),
   ...
   */
  void
  computeDelta(const SharedClique &clique, const SharedClique &parent,
  ...
  SharedConditional marginalFactor(Key j, const Eliminate &function) {
    SharedClique clique{nullptr};

    for (const auto &root : roots_) {
      auto const &cliques = findCliquesByKeys(KeySet{j}, root.second);
      if (cliques.find(j) != cliques.end()) {
        clique = cliques.at(j);
        break;
      }
    }
    ...
    if (clique == nullptr) {
      throw std::runtime_error("Variable not found in the tree");
    }

    FactorGraphType clique_marginal = clique->marginal2();

    BayesNetType marginal_bn;
    try {
      marginal_bn =
          *clique_marginal.marginalMultifrontalBayesNet(Ordering{j}, function);
    } catch (const std::exception &e) {
      std::stringstream ss;
      ss << "Error in marginalMultifrontalBayesNet: " << e.what() << std::endl;
      for (auto const &factor : clique_marginal) {
        ss << "Factor: ";
        for (auto const &key : factor->keys()) {
...
protected:
  ...
  SharedClique
  CopyFromJTByNode(const typename JunctionTreeType::sharedCluster jt_node);
   ...
   */
  void
  eliminateNodeTopDown(const SharedClique &clique, const SharedClique &child,
                       const FactorGraphType &graph, const VariableIndex &vi,
                       const Eliminate &elimination_function,
                       boost::optional<EdgeSet &> boundary_edges = boost::none);
